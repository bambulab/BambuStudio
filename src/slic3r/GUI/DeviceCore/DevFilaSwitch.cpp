...
DevFilaSwitch::DevFilaSwitch(MachineObject* owner)
{
    m_owner = owner;
}
...
std::optional<DevAmsTray> DevFilaSwitch::GetInA_Slot() const
{
    ...
    if (m_owner && ams_id_opt.has_value()) {
        return m_owner->get_tray(std::to_string(ams_id_opt->first), std::to_string(ams_id_opt->first));
    }
...
std::optional<DevExtder> DevFilaSwitch::GetOutA_Extruder() const
{
    ...
    if (m_owner && ext_id_opt.has_value()) {
        return m_owner->GetExtderSystem()->GetExtderById(ext_id_opt.value());
    }
...
void DevFilaSwitch::ParseFilaSwitchInfo(const nlohmann::json& print_jj)
{
    ...
    if (print_jj.contains("device") && print_jj["device"].contains("fila_switch")) {
        ...
        if (fila_switch_jj.contains("stat")) {
            m_cali_status = DevJsonValParser::GetVal<CaliStatus>(fila_switch_jj, "stat");
        }
