...
void ExtruderPanel::update_ams()
{
    ...
    if (!m_ext.empty()) {
        while (m_ext_previews.size() < m_ext.size()) {
            auto ext_preview = new AMSPreview(parent_window, wxID_ANY, AMSinfo(), AMSModel::N3S_AMS);
            ext_preview->Close();
        ...
        if (m_ext_previews.empty()) {
            auto ext_preview = new AMSPreview(parent_window, wxID_ANY, AMSinfo(), AMSModel::EXT_AMS);
            ext_preview->Close();
            m_ext_previews.push_back(ext_preview);
        }
    ...
    : ExtruderPanel(parent, type), m_nozzle_configs(cfg)
{
    ...
    for (size_t i = 0; i < 4; ++i) {
        auto preview = new AMSPreview(ams_container_panel, wxID_ANY, AMSinfo(), AMSModel::GENERIC_AMS);
        preview->Close();
    ...
    for (size_t i = 0; i < 8; ++i) {
        auto preview = new AMSPreview(ams_container_panel, wxID_ANY, AMSinfo(), AMSModel::N3S_AMS);
        preview->Close();
        preview->Bind(wxEVT_ENTER_WINDOW, [this, preview](wxMouseEvent &evt) { ShowAMSDeletePopup(preview); });
        m_ams_previews.push_back(preview);
        m_ams_sizer->Add(preview, 0, wxALIGN_CENTER_VERTICAL | wxRIGHT | wxBOTTOM, FromDIP(2));
