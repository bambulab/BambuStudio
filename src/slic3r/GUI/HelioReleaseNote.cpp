    ...
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, wxString("Helio Additive"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    config_limits[0] = _L("Helio default (recommended)");
    config_limits[1] = _L("Slicer default");
    // Create Limits dropdown with wider width to fit "Helio default (recommended)" text
    auto limits = create_combo_item(panel_optimization, "limits", _L("Limits"), config_limits, 0, 350);

    // Create panel for velocity and volumetric speed fields - visibility controlled by Limits selection
    wxPanel* panel_velocity_volumetric = new wxPanel(panel_optimization);
    if (wxGetApp().dark_mode()) {
    ...
    m_combo_items["limits"]->Bind(wxEVT_COMBOBOX, [=](wxCommandEvent& e) {
        int selection = m_combo_items["limits"]->GetSelection();
        bool show_fields = (selection == 1); // Show when "Slicer default" (index 1) is selected
        if (show_fields) {
...
void HelioInputDialog::update_action(int action)
{
    ...
    if (action == 0) {
        ...
        // Note: Advanced settings are now always visible, so no special handling needed
...
{
    // Advanced settings are now always visible, so this function does nothing
    Layout();
    Fit();
}
...
Slic3r::HelioQuery::OptimizationInput HelioInputDialog::get_optimization_input(bool& ok)
{
    ...
    if (limits_selection == 1) { // "Slicer default" selected
