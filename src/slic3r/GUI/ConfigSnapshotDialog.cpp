...
{    
    ...
    if (has_fff || ! has_sla) {
        text += _(L(PRESET_PRINT_NAME)) + ": " + snapshot.print + "<br>";
        text += _(L(PRESET_FILAMENT_NAME)) + ": " + snapshot.filaments.front() + "<br>";
    }
    if (has_sla) {
        text += _(L("SLA print")) + ": " + snapshot.sla_print + "<br>";
        text += _(L("SLA material")) + ": " + snapshot.sla_material + "<br>";
    }
    text += _(L(PRESET_PRINTER_NAME)) + ": " + (snapshot.physical_printer.empty() ? snapshot.printer : snapshot.physical_printer) + "<br>";
