...
namespace Slic3r
{
    namespace GUI
    {
        ...
        static std::unordered_map<std::string, std::string> system_filament_types_map = {{"PLA", "PLA"}, {"ABS", "ABS"}, {"TPU", "TPU"}, {"PC", "PC"}, {"ASA", "ASA"}, {"PA-CF", "PA-CF"}, {"PA6-CF", "PA6-CF"}, {"PET-CF", "PET-CF"}, {"PETG", "PETG"}, {"PETG-CF", "PETG-CF"}, {"PLA Aero", "PLA-AERO"}, {"PLA-CF", "PLA-CF"}, {"PPA-CF", "PPA-CF"}, {"PPA-GF", "PPA-GF"}, {"PA", "PA"}, {"HIPS", "HIPS"}, {"PPS", "PPS"}, {"PPS-CF", "PPS-CF"}, {"PVA", "PVA"}};
        ...
        static const std::unordered_map<std::string, std::vector<std::string>> printer_model_map =
            {{"Anycubic", {"Kossel Linear Plus", "Kossel Pulley(Linear)", "Mega Zero", "i3 Mega", "Predator"}},
             ...
             {"BIBO", {"BIBO2 Touch"}},
             {"BIQU", {"BX"}},
             {"Creality ENDER", {"Ender-3", "Ender-3 BLTouch", "Ender-3 Pro", "Ender-3 Neo", "Ender-3 V2 Neo", "Ender-3 S1 Plus", "Ender-3 Max", "Ender-3 Max Neo", "Ender-4", "Ender-5 Pro", "Ender-5 Pro", "Ender-7", "Ender-2", "Ender-2 Pro"}},
             {"Creality CR", {"CR-5 Pro", "CR-5 Pro H", "CR-10 SMART", "CR-10 SMART Pro", "CR-10 Mini", "CR-10", "CR-10 v3", "CR-10 S", "CR-10 v2", "CR-10 v2", "CR-10 S Pro", "CR-10 S Pro v2", "CR-10 S4", "CR-10 S5", "CR-20", "CR-20 Pro", "CR-200B", "CR-8"}},
             ...
             {"Geeetech", {"Thunder", "Thunder Pro", "Mizar s", "Mizar Pro", "Mizar", "Mizar Max", "Mizar M", "A10 Pro", "A10 M", "A10 T", "A20", "A20 M", "A20T", "A30 Pro", "A30 M", "A30 T", "E180", "Me Ducer", "Me creator", "Me Creator2", "GiantArmD200", "l3 ProB", "l3 Prow", "l3 ProC"}},
             ...
             {"MakerGear", {"Micro", "M2(V4 Hotend)", "M2 Dual", "M3-single Extruder", "M3-Independent Dual Rev.0", "M3-Independent Dual Rev.0(Duplication Mode)", "M3-Independent Dual Rev.1", "M3-Independent Dual Rev.1(Duplication Mode)", "ultra One", "Ultra One (DuplicationMode)"}},
             ...
             {"TriLAB", {"AzteQ Industrial", "AzteQ Dynamic", "DeltiQ 2", "DeltiQ 2 Plus", "DeltiQ 2 + FlexPrint 2", "DeltiQ 2 Plus + FlexPrint 2", "DeltiQ 2 +FlexPrint", "DeltiQ 2 Plus + FlexPrint", "DeltiQ M", "DeltiQ L", "DeltiQ XL"}},
        ...
        static std::unordered_map<std::string, float> nozzle_diameter_map = {{"0.2", 0.2}, {"0.25", 0.25}, {"0.3", 0.3}, {"0.35", 0.35}, {"0.4", 0.4}, {"0.5", 0.5}, {"0.6", 0.6}, {"0.75", 0.75}, {"0.8", 0.8}, {"1.0", 1.0}, {"1.2", 1.2}};
        ...
        wxBoxSizer *CreateFilamentPresetDialog::create_item(FilamentOptionType option_type)
        {
            ...
            {
            case VENDOR:
                return create_vendor_item();
            case TYPE:
                return create_type_item();
            case SERIAL:
                return create_serial_item();
            case FILAMENT_PRESET:
                return create_filament_preset_item();
            case PRESET_FOR_PRINTER:
                return create_filament_preset_for_printer_item();
            default:
                return nullptr;
            }
        }
        ...
        wxArrayString CreateFilamentPresetDialog::get_filament_preset_choices()
        {
            ...
            for (std::pair<std::string, Preset *> filament_presets : m_all_presets_map)
            {
                ...
                if (inherit && !inherit->value.empty())
                    continue;
                if (std::string::npos == filament_presets.first.find(type_name))
                    continue;
        ...
        wxBoxSizer *CreatePrinterPresetDialog::create_hot_bed_stl_item(wxWindow *parent)
        {
            ...
            m_button_bed_stl->Bind(wxEVT_BUTTON, ([this](wxCommandEvent &e)
                                                  { load_model_stl(); }));
        ...
        wxBoxSizer *CreatePrinterPresetDialog::create_hot_bed_svg_item(wxWindow *parent)
        {
            ...
            m_button_bed_svg->Bind(wxEVT_BUTTON, ([this](wxCommandEvent &e)
                                                  { load_texture(); }));
        ...
        show_page2(); });
            ...
            m_button_page1_cancel->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent &e)
                                        { EndModal(wxID_CANCEL); });
        ...
        bool CreatePrinterPresetDialog::load_system_and_user_presets_with_curr_model(PresetBundle &temp_preset_bundle, bool just_template)
        {
            ...
            const Preset *temp_printer_preset = is_custom_vendor ? temp_preset_bundle.printers.find_custom_preset_by_model_and_variant(m_printer_preset_model_selected.id, varient) : temp_preset_bundle.printers.find_system_preset_by_model_and_variant(m_printer_preset_model_selected.id, varient);
        ...
        void CreatePrinterPresetDialog::update_preset_list_size()
        {
            ...
            if (pos_y + size_y > screen_height - FromDIP(50))
            { // FromDIP(50) task bar
        ...
        wxBoxSizer *CreatePrinterPresetDialog::create_page2_btns_item(wxWindow *parent)
        {
            ...
            m_button_page2_back->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent &e)
                                      { show_page1(); });
            ...
            m_button_create->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent &e)
                                  {
                                      ...
                                      else if (curr_selected_preset_type == m_create_type.base_curr_printer)
                                      { // create printer and based on printer
            ...
            m_button_page2_cancel->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent &e)
                                        { EndModal(wxID_CANCEL); });
        ...
        wxArrayString CreatePrinterPresetDialog::printer_preset_sort_with_nozzle_diameter(const VendorProfile &vendor_profile, float nozzle_diameter)
        {
            ...
            std::sort(preset_sort.begin(), preset_sort.end(), [](const std::pair<float, std::string> &a, const std::pair<float, std::string> &b)
                      { return a.first < b.first; });
        ...
        {
            if (preset->type != Preset::Type::TYPE_PRINT && preset->type != Preset::Type::TYPE_FILAMENT)
                return true;
            PresetBundle *preset_bundle = wxGetApp().preset_bundle;
            ...
            if (!printer_preset)
                return true;
            if (!printer_preset->is_bbl_vendor_preset(preset_bundle))
                return true;
            return false;
        }
                ...
                if (i == btn_idx)
                {
                    ...
                    if (export_type == m_exprot_type.preset_bundle)
                    {
                        for (std::pair<std::string, Preset *> preset : m_printer_presets)
                        {
                            ...
                            if (m_filament_presets.find(preset_name) == m_filament_presets.end() && m_process_presets.find(preset_name) == m_process_presets.end() && preset.second->is_system)
                                continue;
                    ...
                    else if (export_type == m_exprot_type.filament_bundle)
                    {
                        for (std::pair<std::string, std::vector<std::pair<std::string, Preset *>>> filament_name_to_preset : m_filament_name_to_presets)
                        {
                            ...
                            if (all_preset_is_compatible_third_printer)
                                continue;
            ...
            for (std::pair<::CheckBox *, std::string> checkbox_filament_name : m_printer_name)
            {
                if (checkbox_filament_name.first->GetValue())
                {
                    ...
                    for (std::pair<std::string, Preset *> printer_name_to_preset : iter->second)
                    {
                        ...
                        if (preset_is_not_compatible_bbl_printer(filament_preset))
                            continue;
            ...
            for (std::pair<::CheckBox *, std::string> checkbox_preset : m_printer_name)
            {
                if (checkbox_preset.first->GetValue())
                {
                    ...
                    if (m_process_presets.end() != iter)
                    {
                        ...
                        {
                            if (preset_is_not_compatible_bbl_printer(process_preset))
                                continue;
        ...
        void ExportConfigsDialog::data_init()
        {
            ...
            for (const Preset &printer_preset : printer_presets)
            {
                ...
                if (!printer_preset.is_visible || "Default Printer" == preset_name)
                    continue;
                if (preset_bundle.printers.select_preset_by_name(preset_name, false))
                {
                    ...
                    for (const Preset &process_preset : process_presets)
                    {
                        if (process_preset.is_system || process_preset.is_default)
                            continue;
                        if (process_preset.is_compatible)
                        {
        ...
        EditFilamentPresetDialog::EditFilamentPresetDialog(wxWindow *parent, FilamentInfomation *filament_info)
            : DPIDialog(parent ? parent : nullptr, wxID_ANY, _L("Edit Filament"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX), m_filament_id(""), m_filament_name(""), m_vendor_name(""), m_filament_type(""), m_filament_serial("")
        {
        ...
        e.Skip(); }));
            ...
            m_ok_btn->Bind(wxEVT_LEFT_DOWN, [this](wxMouseEvent &e)
                           { EndModal(wxID_OK); });
        ...
        CreatePresetForPrinterDialog::CreatePresetForPrinterDialog(wxWindow *parent, std::string filament_type, std::string filament_id, std::string filament_vendor, std::string filament_name)
            : DPIDialog(parent ? parent : nullptr, wxID_ANY, _L("Add Preset"), wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX), m_filament_id(filament_id), m_filament_name(filament_name), m_filament_vendor(filament_vendor), m_filament_type(filament_type)
        {
            ...
            for (const Preset &printer_preset : printer_presets)
            {
                if (printer_preset.is_visible)
                {
                    ...
                    if (m_preset_bundle->printers.select_preset_by_name(printer_preset.name, true))
                    {
                        ...
                        for (const Preset &filament_preset : filament_presets)
                        {
                            if (filament_preset.is_default || !filament_preset.is_compatible)
                                continue;
                            ConfigOptionStrings *filament_types;
                            ...
                            if (filament_type_.empty())
                                filament_type_ = m_filament_type;
        ...
        EndModal(wxID_OK); });
            m_cancel_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent &e)
                               { EndModal(wxID_CANCEL); });
        ...
        wxPanel *PresetTree::get_child_item(wxPanel *parent, std::shared_ptr<Preset> preset, std::string printer_name, int preset_index, bool is_last)
        {
            ...
            edit_preset_btn->Bind(wxEVT_BUTTON, [this, printer_name, preset_index](wxCommandEvent &e)
                                  { wxGetApp().CallAfter([this, printer_name, preset_index]()
                                                         { edit_preset(printer_name, preset_index); }); });
            del_preset_btn->Bind(wxEVT_BUTTON, [this, printer_name, preset_index](wxCommandEvent &e)
                                 { wxGetApp().CallAfter([this, printer_name, preset_index]()
                                                        { delete_preset(printer_name, preset_index); }); });
