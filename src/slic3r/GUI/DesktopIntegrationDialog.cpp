...
bool DesktopIntegrationDialog::is_integrated()
{
    ...
    // confirmation that BambuSlicer.desktop exists
    struct stat buffer;   
...
void DesktopIntegrationDialog::perform_desktop_integration()
{
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i) {
        // Copy icon BambuSlicer.png from resources_dir()/icons to target_dir_icons/icons/
        if (contains_path_dir(target_candidates[i], "icons")) {
            target_dir_icons = target_candidates[i];
            std::string icon_path = GUI::format("%1%/icons/BambuSlicer.png",resources_dir());
            std::string dest_path = GUI::format("%1%/icons/%2%BambuSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
    ...
    if(target_dir_icons.empty()) {
        BOOST_LOG_TRIVIAL(error) << "Copying BambuSlicer icon to icons directory failed.";
    } else 
    	// save path to icon
        app_config->set("desktop_integration_icon_slicer_path", GUI::format("%1%/icons/%2%BambuSlicer%3%.png", target_dir_icons, icon_theme_path, version_suffix));
    ...
    for (size_t i = 0; i < target_candidates.size(); ++i)
    {
        if (contains_path_dir(target_candidates[i], "applications")) {
            ...
            std::string desktop_file = GUI::format(
                ...
                "StartupNotify=false\n"
                "StartupWMClass=bambu-slicer\n", name_suffix, version_suffix, excutable_path);

            std::string path = GUI::format("%1%/applications/BambuSlicer%2%.desktop", target_dir_desktop, version_suffix);
            if (create_desktop_file(path, desktop_file)){
                BOOST_LOG_TRIVIAL(debug) << "BambuSlicer.desktop file installation success.";
                break;
            } else {
            	// write failed - try another path
                BOOST_LOG_TRIVIAL(debug) << "Attempt to BambuSlicer.desktop file installation failed. failed path: " << target_candidates[i];
    ...
    app_config->set("desktop_integration_app_path", GUI::format("%1%/applications/BambuSlicer%2%.desktop", target_dir_desktop, version_suffix));
    ...
    if (platform_flavor() != PlatformFlavor::LinuxOnChromium) {
        ...
        if (!target_dir_icons.empty())
        {
            std::string icon_path = GUI::format("%1%/icons/BambuSlicer-gcodeviewer_192px.png",resources_dir());
            std::string dest_path = GUI::format("%1%/icons/%2%BambuSlicer-gcodeviewer%3%.png", target_dir_icons, icon_theme_path, version_suffix);
            if (copy_icon(icon_path, dest_path))
                // save path to icon
        ...
        std::string desktop_file = GUI::format(
            ...
            "GenericName=3D Printing Software\n"
            "Icon=BambuSlicer-gcodeviewer%2%\n"
        ...
        std::string desktop_path = GUI::format("%1%/applications/BambuSlicerGcodeViewer%2%.desktop", target_dir_desktop, version_suffix);
        if (create_desktop_file(desktop_path, desktop_file))
            ...
            show_error(nullptr, _L("Performing desktop integration failed - could not create Gcodeviewer desktop file. BambuSlicer desktop file was probably created successfully."));
        }
    }
