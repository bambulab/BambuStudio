...
std::pair<std::string, bool> SlicingProcessCompletedEvent::format_error_message() const
{
	...
	try {
        ...
        error = GUI::format("BambuStudio has encountered a fatal error: \"%1%\"", ex.what()) + "\n\n" +
        		_u8L("Please save your project and restart BambuStudio. "
...
void BackgroundSlicingProcess::prepare_upload()
{
	...
	if (m_print == m_fff_print) {
        ...
        ThumbnailsList thumbnails = this->render_thumbnails(
        	ThumbnailsParams{current_print()->full_print_config().option<ConfigOptionPoints>("thumbnails")->values, true, true, true, true, this->m_current_plate->get_index()});
