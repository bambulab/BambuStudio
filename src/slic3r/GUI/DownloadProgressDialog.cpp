    ...
    : DPIDialog(static_cast<wxWindow *>(wxGetApp().mainframe), wxID_ANY, title, wxDefaultPosition, wxDefaultSize, wxCAPTION | wxCLOSE_BOX)
{
    ...
    m_worker = std::make_unique<WindowWorker<BoostThreadWorker>>(this, m_status_bar, "download_worker");
...
bool DownloadProgressDialog::Show(bool show)
{
    if (show) {
        ...
        auto upgrade_job = make_job();
        upgrade_job->set_event_handle(this);
        ...
        replace_job(*m_worker, std::move(upgrade_job));
    }
