...
/*	else {
	...
	if (is_editor) {
...
#if __APPLE__
		def.label = L("Allow just a single BambuStudio instance");
		def.type = coBool;
		...
		def.label = L("Allow just a single BambuStudio instance");
		def.type = coBool;
		def.tooltip = L("If this is enabled, when starting BambuStudio and another instance of the same BambuStudio is already running, that instance will be reactivated instead.");
#endif
