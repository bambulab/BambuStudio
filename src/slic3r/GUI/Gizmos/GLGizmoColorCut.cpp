...
#include "ag/customcut/ColorCutCoordinator.hpp"
...
static const ModelVolume *find_single_colorcut_source_volume(const ModelObject &object)
{
...
static void show_colorcut_warning(const Slic3r::ColorCut::ColorCutResult &result)
{
    if (result.warnings.empty())
        return;

    MessageDialog dlg(nullptr, from_u8(result.warnings.front().message), _L("ColorCut"), wxOK | wxICON_WARNING);
    dlg.ShowModal();
}

static void append_groove_debug_line(const std::string &message)
{
    wxFileName executable_path(wxStandardPaths::Get().GetExecutablePath());
    executable_path.SetFullName("groove_debug.txt");
    std::ofstream log_file(executable_path.GetFullPath().ToUTF8().data(), std::ios::out | std::ios::app);
    if (!log_file.is_open())
        return;
    log_file << message << std::endl;
}

static std::string colorcut_gizmo_log_path()
{
    wxFileName executable_path(wxStandardPaths::Get().GetExecutablePath());
    executable_path.SetFullName("colorcut.log");
    return executable_path.GetFullPath().ToUTF8().data();
}

static void colorcut_gizmo_log(const std::string &message)
{
    static std::mutex s_mutex;
    std::lock_guard<std::mutex> lock(s_mutex);
    std::ofstream out(colorcut_gizmo_log_path(), std::ios::app);
    if (out)
        out << message << std::endl;
}

static void colorcut_gizmo_log_phase(const std::string &phase_name)
{
    colorcut_gizmo_log("");
    colorcut_gizmo_log("****************************************");
    colorcut_gizmo_log(phase_name);
    colorcut_gizmo_log("****************************************");
}

static void colorcut_gizmo_log_reset()
{
    static std::mutex s_mutex;
    std::lock_guard<std::mutex> lock(s_mutex);
    std::ofstream out(colorcut_gizmo_log_path(), std::ios::trunc);
}

static Slic3r::ColorCut::SurfacePlaneDefinition make_surface_plane(const Transform3d &matrix)
{
    Slic3r::ColorCut::SurfacePlaneDefinition plane;
    plane.point = matrix * Vec3d::Zero();
    plane.normal = (matrix.linear() * Vec3d::UnitZ()).normalized();
    return plane;
}

static std::vector<Slic3r::ColorCut::SurfacePlaneDefinition> build_groove_surface_planes(
    const Groove &groove,
    const Transform3d &cut_matrix,
    const Transform3d &rotation_m)
{
    std::vector<Slic3r::ColorCut::SurfacePlaneDefinition> planes;
    planes.reserve(7);
    ...
    const Matrix3d rotation_linear = rotation_m.linear();

    planes.emplace_back(make_surface_plane(Geometry::translation_transform(rotation_linear * (groove_half_depth * Vec3d::UnitZ())) * cut_matrix));
    planes.emplace_back(make_surface_plane(Geometry::translation_transform(rotation_linear * (-groove_half_depth * Vec3d::UnitZ())) * cut_matrix));
    planes.emplace_back(make_surface_plane(Geometry::translation_transform(rotation_linear * (-h_side_shift * Vec3d::UnitX())) * cut_matrix * Geometry::rotation_transform(Vec3d(0, -groove.flaps_angle, -groove.angle))));
    planes.emplace_back(make_surface_plane(Geometry::translation_transform(rotation_linear * (h_side_shift * Vec3d::UnitX())) * cut_matrix * Geometry::rotation_transform(Vec3d(0, groove.flaps_angle, groove.angle))));
    planes.emplace_back(make_surface_plane(Geometry::translation_transform(rotation_linear * (-h_groove_shift_tolerance * Vec3d::UnitZ())) * cut_matrix));
    planes.emplace_back(make_surface_plane(Geometry::translation_transform(rotation_linear * (-h_side_shift_tolerance * Vec3d::UnitX())) * cut_matrix * Geometry::rotation_transform(Vec3d(0, -groove.flaps_angle, -groove.angle))));
    planes.emplace_back(make_surface_plane(Geometry::translation_transform(rotation_linear * (h_side_shift_tolerance * Vec3d::UnitX())) * cut_matrix * Geometry::rotation_transform(Vec3d(0, groove.flaps_angle, groove.angle))));
    return planes;
}

static size_t count_mmu_tagged_triangles(const ModelVolume &volume)
{
    size_t tagged_triangles = 0;
    for (size_t triangle_index = 0; triangle_index < volume.mesh().its.indices.size(); ++triangle_index)
        if (!volume.mmu_segmentation_facets.get_triangle_as_string(static_cast<int>(triangle_index)).empty())
            ++tagged_triangles;
    return tagged_triangles;
}

static void log_cut_result_summary(const char *label, const ModelObjectPtrs &objects)
{
    Slic3r::ColorCut::ColorCutAttributeRepository &repository = Slic3r::ColorCut::global_color_cut_attribute_repository();

    append_groove_debug_line("GLGizmoColorCut " + std::string(label) + ": objects=" + std::to_string(objects.size()));
    for (size_t object_index = 0; object_index < objects.size(); ++object_index) {
        const ModelObject *object = objects[object_index];
        if (object == nullptr) {
            append_groove_debug_line("GLGizmoColorCut " + std::string(label) + " object[" + std::to_string(object_index) + "]=null");
            continue;
        }

        append_groove_debug_line("GLGizmoColorCut " + std::string(label) + " object[" + std::to_string(object_index) + "] name=" + object->name + " volumes=" + std::to_string(object->volumes.size()));
        for (size_t volume_index = 0; volume_index < object->volumes.size(); ++volume_index) {
            ...
            const auto external_color_data = repository.get_volume_color_data(volume->id().id);
            append_groove_debug_line(
                "GLGizmoColorCut " + std::string(label) +
                " object[" + std::to_string(object_index) + "] volume[" + std::to_string(volume_index) +
                "] id=" + std::to_string(volume->id().id) +
                " from_upper=" + std::to_string(volume->is_from_upper() ? 1 : 0) +
                " triangles=" + std::to_string(volume->mesh().its.indices.size()) +
                " mmu_tagged=" + std::to_string(count_mmu_tagged_triangles(*volume)) +
                " has_repo_color=" + std::to_string(external_color_data.has_value() ? 1 : 0));
        }
    }
}

static bool all_model_part_volumes_have_preserved_appearance(const ModelObjectPtrs &objects)
{
    Slic3r::ColorCut::ColorCutAttributeRepository &repository = Slic3r::ColorCut::global_color_cut_attribute_repository();
    ...
    for (const ModelObject *object : objects) {
        ...
        for (const ModelVolume *volume : object->volumes) {
            ...
            has_model_part = true;
            if (count_mmu_tagged_triangles(*volume) == 0 && !repository.get_volume_color_data(volume->id().id).has_value())
                return false;
        }
    }
...
static bool any_model_part_volume_has_preserved_appearance(const ModelObjectPtrs &objects)
{
    Slic3r::ColorCut::ColorCutAttributeRepository &repository = Slic3r::ColorCut::global_color_cut_attribute_repository();
...
const double GLGizmoColorCut::Offset = 20.0;
const double GLGizmoColorCut::Margin = 20.0;
const std::array<float, 4> GLGizmoColorCut::GrabberColor      = { 1.0, 1.0, 0.0, 1.0 };
const std::array<float, 4> GLGizmoColorCut::GrabberHoverColor = { 0.7, 0.7, 0.0, 1.0};

GLGizmoColorCut::GLGizmoColorCut(GLCanvas3D& parent, unsigned int sprite_id)
...
bool GLGizmoColorCut::gizmo_event(SLAGizmoEventType action, const Vec2d &mouse_position, bool shift_down, bool alt_down, bool control_down)
{
...
bool GLGizmoColorCut::on_key(const wxKeyEvent& evt)
{
...
std::string GLGizmoColorCut::get_tooltip() const
{
    std::string tooltip;
...
BoundingBoxf3 GLGizmoColorCut::bounding_box() const
{
    BoundingBoxf3                 ret;
...
BoundingBoxf3 GLGizmoColorCut::transformed_bounding_box(const Vec3d &plane_center, const Transform3d &rotation_m) const
{
    const Selection &selection = m_parent.get_selection();
...
bool GLGizmoColorCut::is_looking_forward() const
{
...
bool GLGizmoColorCut::unproject_on_cut_plane(const Vec2d &mouse_pos, Vec3d &pos, Vec3d &pos_world, bool respect_contours)
{
...
static void clear_temp_cut_surface_markers(const ModelObjectPtrs &objects)
{
...
void GLGizmoColorCut::reset_cut_plane()
{
...
void GLGizmoColorCut::reset_all()
{
...
BoundingBoxf3 GLGizmoColorCut::get_bounding_box() const
{
...
std::string GLGizmoColorCut::get_icon_filename(bool b_dark_mode) const
{
    return b_dark_mode ? "toolbar_colorcut_dark.svg" : "toolbar_colorcut.svg";
}

bool GLGizmoColorCut::on_init()
{
...
std::string GLGizmoColorCut::on_get_name() const
{
    if (!on_is_activable() && m_state == EState::Off) {
        return _u8L("ColorCut") + ":\n" + _u8L("Please select single object.");
    } else {
        return _u8L("ColorCut");
    }
}

void GLGizmoColorCut::apply_color_clip_plane_colors()
{
...
void GLGizmoColorCut::on_load(cereal::BinaryInputArchive &ar)
{
    size_t mode;
    float  groove_depth;
    float  groove_width;
...
void GLGizmoColorCut::on_save(cereal::BinaryOutputArchive &ar) const
{
...
void GLGizmoColorCut::data_changed(bool is_serializing)
{
    reset_rotation();
    if (m_hover_id < 0) { // BBL
        update_bb();
...
void GLGizmoColorCut::on_set_state()
{
    ...
    if (get_state() == On) {
        ...
        if (m_part_selection && m_part_selection->valid()) {
            wxGetApp().CallAfter([this]() {
                m_part_selection.reset(new ColorCutPartSelection());
            });
        }
    }
}

void GLGizmoColorCut::close()
{//close gizmo == open it again
    auto &mng = m_parent.get_gizmos_manager();
    if (mng.get_current_type() == GLGizmosManager::ColorCut)
        mng.open_gizmo(GLGizmosManager::ColorCut);
}

bool GLGizmoColorCut::on_is_activable() const
{
    const Selection &selection  = m_parent.get_selection();
...
CommonGizmosDataID GLGizmoColorCut::on_get_requirements() const
{
...
void GLGizmoColorCut::on_start_dragging()
{
...
void GLGizmoColorCut::on_stop_dragging()
{
...
void GLGizmoColorCut::update_plate_center(Axis axis_type, double projection, bool is_abs_move) {
...
void GLGizmoColorCut::update_plate_normal_boundingbox_clipper(const Transform3d &rotation_tmp)
{
...
void GLGizmoColorCut::on_update(const UpdateData& data)
{
    if (m_hover_id < 0)
        return;
    m_is_dragging = true;
...
void GLGizmoColorCut::on_render()
{
    if (m_state == On) {
...
void GLGizmoColorCut::on_render_for_picking()
{
...
void GLGizmoColorCut::on_render_input_window(float x, float y, float bottom_limit)
{
...
void GLGizmoColorCut::show_tooltip_information(float x, float y)
{
...
static void update_object_cut_id(CutObjectBase &cut_id, ModelObjectCutAttributes attributes, const int dowels_count)
{
...
static void synchronize_model_after_cut(Model &model, const CutObjectBase &cut_id)
{
    for (ModelObject *obj : model.objects)
        if (obj->is_cut() && obj->cut_id.has_same_id(cut_id) && !obj->cut_id.is_equal(cut_id)) obj->cut_id.copy(cut_id);
}

void GLGizmoColorCut::perform_cut(const Selection& selection)
{
    ...
    wxCHECK_RET(instance_idx >= 0 && object_idx >= 0, "GLGizmoColorCut: Invalid object selection");

    Plater *     plater = wxGetApp().plater();
    ModelObject *mo     = plater->model().objects[object_idx];
    if (!mo) return;

    // Reset colorcut.log at the very start of each cut attempt so every run
    // produces a clean, single-attempt diagnostic trace. All downstream layers
    // (coordinator, legacy backend, cut-time mesh-repair replay, GUI-level
    // reapply / marked-surface override) only append to this file.
    colorcut_gizmo_log_reset();
    colorcut_gizmo_log_phase("PERFORM_CUT ENTRY");
    {
        const Transform3d cut_m = get_cut_matrix(selection);
        const Vec3d cut_origin = cut_m * Vec3d::Zero();
        const Vec3d cut_normal = cut_m.linear() * Vec3d::UnitZ();
        const Vec3d euler = Geometry::extract_euler_angles(cut_m);
        std::ostringstream stream;
        stream << "object: '" << mo->name << "'"
               << "\nobject_index: " << object_idx
               << "\ninstance_index: " << instance_idx
               << "\ncut_mode: " << static_cast<int>(m_cut_mode)
               << "\nkeep_upper: " << (m_keep_upper ? "TRUE" : "FALSE")
               << "\nkeep_lower: " << (m_keep_lower ? "TRUE" : "FALSE")
               << "\ncut_to_parts: " << (m_cut_to_parts ? "TRUE" : "FALSE")
               << "\nplace_on_cut_upper: " << (m_place_on_cut_upper ? "TRUE" : "FALSE")
               << "\nplace_on_cut_lower: " << (m_place_on_cut_lower ? "TRUE" : "FALSE")
               << "\nrotate_upper: " << (m_rotate_upper ? "TRUE" : "FALSE")
               << "\nrotate_lower: " << (m_rotate_lower ? "TRUE" : "FALSE")
               << "\nsingle_cut_surface_color: " << (m_single_cut_surface_color ? "TRUE" : "FALSE")
               << "\ncut_origin: (" << cut_origin.x() << ", " << cut_origin.y() << ", " << cut_origin.z() << ")"
               << "\ncut_normal: (" << cut_normal.x() << ", " << cut_normal.y() << ", " << cut_normal.z() << ")"
               << "\ncut_euler_xyz_deg: ("
               << (euler.x() * (180.0 / 3.14159265358979323846)) << ", "
               << (euler.y() * (180.0 / 3.14159265358979323846)) << ", "
               << (euler.z() * (180.0 / 3.14159265358979323846)) << ")";
        colorcut_gizmo_log(stream.str());
    }
    ...
    // perform cut
    {
        ...
        const ModelObjectPtrs *new_objects = nullptr;

        auto finalize_cut_result = [&](const ModelObjectPtrs &objects, ProgressDialog *progress_dialog, bool force_scene_refresh) {
            if (progress_dialog != nullptr) {
                progress_dialog->Update(90, _L("Validating results..."));
                wxGetApp().Yield();
            }

            check_objects_after_cut(objects);// Fix for #11487 - Cut Connectors Broken when assigning part to other side
            ...
            if (is_windows10()) {
                ...
                for (size_t i = 0; i < objects.size(); i++) {
                    for (size_t j = 0; j < objects[i]->volumes.size(); j++) {
                        ModelVolume *volume = objects[i]->volumes[j];
                        if (volume == nullptr || !volume->is_model_part() || volume->is_cut_connector())
                            continue;

                        const auto &mesh_stats = volume->mesh().stats();
                        if (mesh_stats.open_edges > 0 || objects[i]->get_repaired_errors_count(int(j)) > 0) {
                            if (!is_showed_dialog) {
                            ...
                            ProgressDialog progress_dlg(_L("Repairing model object"), "", 100, find_toplevel_parent(plater), wxPD_AUTO_HIDE | wxPD_APP_MODAL, true);

                            auto model_name = objects[i]->name;
                            if (!fix_and_update_progress(objects[i], j, model_name, progress_dlg, succes_models, failed_models)) {
            ...
            plater->apply_cut_object_to_model(object_idx, objects);
            synchronize_model_after_cut(plater->model(), cut_id);

            if (force_scene_refresh) {
                if (progress_dialog != nullptr) {
        ...
        const ModelVolume *colorcut_source_volume = find_single_colorcut_source_volume(*cut_mo);
        const bool try_colorcut = !cut_by_contour && !cut_with_groove && !has_connectors && colorcut_source_volume != nullptr;
        if (try_colorcut) {
            ...
            Slic3r::ColorCut::ColorCutCoordinator coordinator;
            Slic3r::ColorCut::ColorCutRequest request;
            request.object = cut_mo;
            request.object_index = object_idx;
            request.instance_index = instance_idx;
            request.cut_matrix = cut_matrix;
            request.attributes = attributes;
            request.mode = Slic3r::ColorCut::ColorCutMode::LegacyPlane;
            ...
            std::optional<Slic3r::ColorCut::ColorCutResult> colorcut_result;
            Slic3r::ColorCut::ColorCutAppearanceSnapshotBuilder snapshot_builder;
            Slic3r::ColorCut::ObjectAppearanceSnapshot snapshot = snapshot_builder.build(*cut_mo);
            auto start_time = std::chrono::steady_clock::now();

            // --- Launch worker thread ---
            std::thread worker([&]() {
                try {
                    colorcut_result = coordinator.execute(request);

                    if (progress->canceled.load()) { progress->finished = true; return; }

                    if (colorcut_result.has_value() && colorcut_result->handled && !colorcut_result->new_objects.empty()) {
                        {
                            std::lock_guard<std::mutex> lk(progress->mutex);
                        ...
                        // The coordinator's geometry backend already runs cut-time
                        // ColorCut::IntegrationBridge::reapply_after_mesh_repair(...) for each
                        // produced volume in the original (upright) instance frame. After that,
                        // reset_instance_transformation()/place_on_cut may rotate the new parts.
                        // Re-running reapply_from_single_source(...) here would compare the still-
                        // upright source mesh against the now-reoriented target mesh, classifying
                        // most triangles as normal-incompatible cap and destroying the colors that
                        // were already correctly transferred at cut time. This was the cause of the
                        // -38 degree planar regression where ~83% of triangles ended up class_4.
                        //
                        // Mirror the legacy path's gating: if the cut already produced preserved
                        // appearance (which the cut-time replay just did), skip the late-frame
                        // reapply. When single-cut-surface-color is on, recolor only the cap
                        // triangles via the marked-triangle override which works in target space
                        // and is therefore orientation-safe.
                        const bool cut_has_any_preserved_appearance =
                            any_model_part_volume_has_preserved_appearance(colorcut_result->new_objects);

                        Slic3r::ColorCut::ColorCutAttributeTransfer transfer;
                        if (cut_has_any_preserved_appearance) {
                            if (m_single_cut_surface_color) {
                                colorcut_gizmo_log_phase("CALLING apply_uniform_color_to_marked_triangles (coordinator path)");
                                transfer.apply_uniform_color_to_marked_triangles(snapshot, *colorcut_source_volume, colorcut_result->new_objects);
                                colorcut_gizmo_log("apply_uniform_color_to_marked_triangles COMPLETE");
                            } else {
                                colorcut_gizmo_log("REAPPLY SKIPPED (coordinator path): cut-time replay already preserved appearance");
                            }
                        } else {
                            colorcut_gizmo_log_phase("CALLING reapply_from_single_source (coordinator path fallback)");
                            transfer.reapply_from_single_source(snapshot, *cut_mo, *colorcut_source_volume, instance_idx, colorcut_result->new_objects, m_single_cut_surface_color, {}, true);
                            colorcut_gizmo_log("reapply_from_single_source COMPLETE");
                        }
...

                        {
                            ...
                            progress->phase = "Postprocessing cut faces";
                            progress->updated = true;
                            progress->cv.notify_all();
                        }
                    }
                    ...
                    {
                        std::lock_guard<std::mutex> lk(progress->mutex);
                        progress->fraction = 0.90f;
                        progress->phase = "Cut complete";
                        progress->updated = true;
                        progress->cv.notify_all();
                    }
                } catch (...) {}
            ...
            if (progress->canceled.load()) {
                // Canceled — close dialog and fall through to legacy path
                progress_dialog.Update(99, _L("Canceled"));
                wxGetApp().Yield();
                progress_dialog.Show(false);
                wxGetApp().Yield();
            } else if (colorcut_result.has_value() &&
                       colorcut_result->handled && !colorcut_result->new_objects.empty()) {
                finalize_cut_result(colorcut_result->new_objects, &progress_dialog, true);
                show_colorcut_warning(*colorcut_result);
                return;
            } else {
                // Failed — close dialog and fall through to legacy path
                progress_dialog.Update(99, _L("Done"));
                wxGetApp().Yield();
                progress_dialog.Show(false);
                wxGetApp().Yield();
            }
            // Canceled or failed — fall through to legacy cut path below
        }

        Cut cut(cut_mo, instance_idx, cut_matrix, attributes);
        cut.set_offset_for_two_part = true;
        if (new_objects == nullptr) {
            colorcut_gizmo_log_phase("GIZMO CUT EXECUTION (fallback path)");
            {
                std::ostringstream stream;
                stream << "object: " << cut_mo->name
                       << "\ninstance: " << instance_idx
                       << "\ncut_by_contour: " << (cut_by_contour ? "TRUE" : "FALSE")
                       << "\ncut_with_groove: " << (cut_with_groove ? "TRUE" : "FALSE")
                       << "\nhas_connectors: " << (has_connectors ? "TRUE" : "FALSE")
                       << "\ntry_colorcut: " << (try_colorcut ? "TRUE" : "FALSE")
                       << "\nm_single_cut_surface_color: " << (m_single_cut_surface_color ? "TRUE" : "FALSE")
                       << "\ncolorcut_source_volume: " << (colorcut_source_volume != nullptr ? colorcut_source_volume->name : "nullptr");
                colorcut_gizmo_log(stream.str());
            }

            if (cut_with_groove) {
                append_groove_debug_line("GLGizmoColorCut groove cut start: object=" + cut_mo->name +
                                         " instance=" + std::to_string(instance_idx) +
                                         " colorcut_source=" + std::to_string(colorcut_source_volume != nullptr ? 1 : 0));
            }

            if (cut_by_contour)
                new_objects = &cut.perform_by_contour(m_part_selection->get_cut_parts(), dowels_count);
            else if (cut_with_groove) {
                colorcut_gizmo_log("Executing perform_with_groove...");
                new_objects = &cut.perform_with_groove(m_groove, m_rotate_matrix);
            } else {
                colorcut_gizmo_log("Executing perform_with_plane...");
                new_objects = &cut.perform_with_plane();
            }

            colorcut_gizmo_log("Cut returned " + std::to_string(new_objects != nullptr ? new_objects->size() : 0) + " objects");

            const bool cut_has_complete_preserved_appearance =
                new_objects != nullptr && all_model_part_volumes_have_preserved_appearance(*new_objects);
            const bool cut_has_any_preserved_appearance =
                new_objects != nullptr && any_model_part_volume_has_preserved_appearance(*new_objects);
            const bool groove_has_preserved_appearance = cut_with_groove && cut_has_complete_preserved_appearance;
            const bool planar_has_preserved_appearance = !cut_by_contour && !cut_with_groove && cut_has_any_preserved_appearance;
            const bool use_planar_single_color_postprocess = m_single_cut_surface_color && !cut_by_contour && (try_colorcut || has_connectors);
            const bool use_groove_single_color_postprocess = m_single_cut_surface_color && cut_with_groove;

            colorcut_gizmo_log_phase("REAPPLY DECISION");
            {
                std::ostringstream stream;
                stream << "cut_has_complete_preserved_appearance: " << (cut_has_complete_preserved_appearance ? "TRUE" : "FALSE")
                       << "\ncut_has_any_preserved_appearance: " << (cut_has_any_preserved_appearance ? "TRUE" : "FALSE")
                       << "\ngroove_has_preserved_appearance: " << (groove_has_preserved_appearance ? "TRUE" : "FALSE")
                       << "\nplanar_has_preserved_appearance: " << (planar_has_preserved_appearance ? "TRUE" : "FALSE")
                       << "\nuse_planar_single_color_postprocess: " << (use_planar_single_color_postprocess ? "TRUE" : "FALSE")
                       << "\nuse_groove_single_color_postprocess: " << (use_groove_single_color_postprocess ? "TRUE" : "FALSE");
                colorcut_gizmo_log(stream.str());
            }

            if ((cut_with_groove || planar_has_preserved_appearance) && new_objects != nullptr)
                log_cut_result_summary("groove-pre-reapply", *new_objects);

            if ((cut_by_contour || cut_with_groove || has_connectors || try_colorcut) && colorcut_source_volume != nullptr && new_objects != nullptr && !new_objects->empty()) {
                if (planar_has_preserved_appearance && use_planar_single_color_postprocess) {
                    colorcut_gizmo_log_phase("CALLING apply_uniform_color_to_marked_triangles");
                    Slic3r::ColorCut::ColorCutAppearanceSnapshotBuilder snapshot_builder;
                    Slic3r::ColorCut::ObjectAppearanceSnapshot snapshot = snapshot_builder.build(*cut_mo);
                    Slic3r::ColorCut::ColorCutAttributeTransfer transfer;
                    transfer.apply_uniform_color_to_marked_triangles(snapshot, *colorcut_source_volume, *new_objects);
                    colorcut_gizmo_log("apply_uniform_color_to_marked_triangles COMPLETE");
                    log_cut_result_summary("planar-post-marked-override", *new_objects);
                } else if (planar_has_preserved_appearance) {
                    colorcut_gizmo_log("REAPPLY SKIPPED: planar cut has preserved appearance");
                } else if (cut_with_groove && groove_has_preserved_appearance && !use_groove_single_color_postprocess) {
                    colorcut_gizmo_log("REAPPLY SKIPPED: groove has preserved appearance and groove_single_color=false");
                    append_groove_debug_line("GLGizmoColorCut groove reapply skipped: preserved staged appearance");
                } else if (cut_with_groove && groove_has_preserved_appearance && use_groove_single_color_postprocess) {
                    colorcut_gizmo_log_phase("CALLING apply_uniform_color_to_marked_triangles");
                    append_groove_debug_line("GLGizmoColorCut groove marked override start: source_volume_id=" + std::to_string(colorcut_source_volume->id().id));
                    Slic3r::ColorCut::ColorCutAppearanceSnapshotBuilder snapshot_builder;
                    Slic3r::ColorCut::ObjectAppearanceSnapshot snapshot = snapshot_builder.build(*cut_mo);
                    Slic3r::ColorCut::ColorCutAttributeTransfer transfer;
                    transfer.apply_uniform_color_to_marked_triangles(snapshot, *colorcut_source_volume, *new_objects);
                    colorcut_gizmo_log("apply_uniform_color_to_marked_triangles COMPLETE");
                    if (cut_with_groove)
                        log_cut_result_summary("groove-post-reapply", *new_objects);
                } else {
                    colorcut_gizmo_log_phase("CALLING reapply_from_single_source");
                    colorcut_gizmo_log("Parameters: uniform_cap_color=" + std::to_string(m_single_cut_surface_color ? 1 : 0)
                        + " planar=" + std::to_string(use_planar_single_color_postprocess ? 1 : 0)
                        + " groove=" + std::to_string(use_groove_single_color_postprocess ? 1 : 0));
                    if (cut_with_groove) {
                    append_groove_debug_line("GLGizmoColorCut groove reapply start: source_volume_id=" + std::to_string(colorcut_source_volume->id().id)
                        + " groove_single_color=" + std::to_string(use_groove_single_color_postprocess ? 1 : 0));
                    }
                    Slic3r::ColorCut::ColorCutAppearanceSnapshotBuilder snapshot_builder;
                    Slic3r::ColorCut::ObjectAppearanceSnapshot snapshot = snapshot_builder.build(*cut_mo);
                    Slic3r::ColorCut::ColorCutAttributeTransfer transfer;
                    transfer.reapply_from_single_source(snapshot, *cut_mo, *colorcut_source_volume, instance_idx, *new_objects, m_single_cut_surface_color, {}, use_planar_single_color_postprocess, use_groove_single_color_postprocess);
                    colorcut_gizmo_log("reapply_from_single_source COMPLETE");
                    if (cut_with_groove)
                        log_cut_result_summary("groove-post-reapply", *new_objects);
                }
            ...
            if (cut_with_groove && new_objects != nullptr)
                clear_temp_cut_surface_markers(*new_objects);
        }

        finalize_cut_result(*new_objects, nullptr, false);
    }
}

bool GLGizmoColorCut::can_perform_cut() const
{
...
void GLGizmoColorCut::apply_connectors_in_model(ModelObject *mo, int &dowels_count) {
...
bool GLGizmoColorCut::is_selection_changed(bool alt_down, bool shift_down)
{
    if (m_hover_id >= c_connectors_group_id) {
        if (alt_down)
...
void GLGizmoColorCut::select_connector(int idx, bool select)
{
...
double GLGizmoColorCut::calc_projection(const Vec3d &drag_pos, const Linef3 &mouse_ray, const Vec3d &project_dir) const
{
...
Vec3d GLGizmoColorCut::get_plane_normal() const {
    return m_plane_normal;
}

Vec3d GLGizmoColorCut::get_plane_center() const {
    return m_plane_center;
}

void GLGizmoColorCut::reset_rotation()
{
    for (int i = 0; i < 3; i++) {
...
void GLGizmoColorCut::put_connectors_on_cut_plane(const Vec3d &cp_normal, double cp_offset)
{
...
void GLGizmoColorCut::update_plane_normal()
{
    // update cut_normal
...
void GLGizmoColorCut::update_clipper()
{
    update_plane_normal();
...
void GLGizmoColorCut::render_cut_plane_and_grabbers()
{
...
void GLGizmoColorCut::on_render_rotate_gizmos() {
...
void GLGizmoColorCut::render_connectors()
{
...
void GLGizmoColorCut::render_clipper_cut()
{
    if (!m_connectors_editing)
...
void GLGizmoColorCut::render_cut_line()
{
...
void GLGizmoColorCut::clear_selection()
{
    m_selected.clear();
    m_selected_count = 0;
}

void GLGizmoColorCut::init_connector_shapes()
{
...
void GLGizmoColorCut::set_connectors_editing(bool connectors_editing)
{
...
void GLGizmoColorCut::reset_connectors()
{
...
void GLGizmoColorCut::update_connector_shape()//update mesh
{
...
void GLGizmoColorCut::apply_selected_connectors(std::function<void(size_t idx)> apply_fn)
{
...
void GLGizmoColorCut::select_all_connectors()
{
    std::fill(m_selected.begin(), m_selected.end(), true);
    m_selected_count = int(m_selected.size());
}

void GLGizmoColorCut::unselect_all_connectors()
{
...
void GLGizmoColorCut::validate_connector_settings()
{
...
bool GLGizmoColorCut::add_connector(CutConnectors &connectors, const Vec2d &mouse_position)
{
...
bool GLGizmoColorCut::delete_selected_connectors()
{
    CutConnectors &connectors = m_c->selection_info()->model_object()->cut_connectors;
    if (connectors.empty())
        return false;
...
bool GLGizmoColorCut::is_outside_of_cut_contour(size_t idx, const CutConnectors &connectors, const Vec3d cur_pos)
{
...
bool GLGizmoColorCut::is_conflict_for_connector(size_t idx, const CutConnectors &connectors, const Vec3d cur_pos)
{
...
void GLGizmoColorCut::switch_to_mode(CutMode new_mode) {
    m_cut_mode = new_mode;
    if (m_cut_mode == CutMode::cutTongueAndGroove) {
...
void GLGizmoColorCut::flip_cut_plane()
{
...
void GLGizmoColorCut::update_plane_model()
{
    m_plane.reset();
    init_picking_models();
}
...
void GLGizmoColorCut::init_picking_models()
{
...
bool GLGizmoColorCut::has_valid_groove() const
{
    if (m_cut_mode != CutMode::cutTongueAndGroove)
        return true;
...
bool GLGizmoColorCut::has_valid_contour() const
{
    const auto clipper = m_c->object_clipper();
    return clipper && clipper->has_valid_contour();
}

void GLGizmoColorCut::reset_cut_by_contours()
{
    update_buffer_data();
    update_plane_normal();
...
void GLGizmoColorCut::process_contours()
{
    const Selection &      selection     = m_parent.get_selection();
    ...
    if (m_cut_mode == CutMode::cutTongueAndGroove) {
        if (has_valid_groove()) {
            ...
            if (!new_objects.empty()) {
                m_part_selection.reset(new ColorCutPartSelection(new_objects.front(), instance_idx));
            }
        }
    } else {
        if (m_c->object_clipper()) {
            m_part_selection.reset(new ColorCutPartSelection(model_objects[object_idx], get_cut_matrix(selection), instance_idx, m_plane_center, m_plane_normal, *m_c->object_clipper()));
        }
    }
...
void GLGizmoColorCut::render_flip_plane_button(bool disable_pred /*=false*/)
{
    ImGui::SameLine();
...
void GLGizmoColorCut::toggle_model_objects_visibility(bool show_in_3d)
{
...
void GLGizmoColorCut::deal_connector_pos_by_type(
...
void GLGizmoColorCut::update_bb()
{
    ...
    if (!m_max_pos.isApprox(box.max) || !m_min_pos.isApprox(box.min)) {
        ...
        if (const int object_idx = m_parent.get_selection().get_object_idx();
            object_idx >= 0 && m_cut_mode != CutMode::cutTongueAndGroove && !wxGetApp().plater()->model().objects[object_idx]->cut_connectors.empty())
...
void GLGizmoColorCut::check_and_update_connectors_state()
{
...
void GLGizmoColorCut::set_center(const Vec3d &center, bool update_tbb)
{
...
bool GLGizmoColorCut::set_center_pos(const Vec3d &center_pos, bool update_tbb)
{
...
void GLGizmoColorCut::invalidate_cut_plane()
{
...
void GLGizmoColorCut::rotate_vec3d_around_plane_center(Vec3d &vec, const Transform3d &rotate_matrix, const Vec3d &center) {
...
Transform3d GLGizmoColorCut::get_cut_matrix(const Selection &selection)
{
    const int    instance_idx = selection.get_instance_idx();
    const int    object_idx   = selection.get_object_idx();
...
void GLGizmoColorCut::update_buffer_data() {
...
bool GLGizmoColorCut::render_cut_mode_combo(double label_width, float item_width)
{
...
void GLGizmoColorCut::render_color_marker(float size, const ColorRGBA &color)
{
    ImGui::SameLine();
...
void GLGizmoColorCut::render_cut_plane_input_window(float x, float y, float bottom_limit)
{
...
void GLGizmoColorCut::init_connectors_input_window_data()
{
    if (!m_c) {
        return;
    }
...
void GLGizmoColorCut::render_connectors_input_window(float x, float y, float bottom_limit)
{
    if (!m_c) {
        return;
    }
...
void GLGizmoColorCut::render_input_window_warning() const
{
...
bool GLGizmoColorCut::render_reset_button(const std::string &label_id, const std::string &tooltip) const
{
...
bool GLGizmoColorCut::render_connect_type_radio_button(CutConnectorType type)
{
...
bool GLGizmoColorCut::render_slider_double_input(const std::string &label, float &value_in, float &tolerance_in)
{
    // -------- [ ] -------- [ ]
...
bool GLGizmoColorCut::render_slider_double_input_by_format(const std::string &label, float &value_in, float value_min, float value_max, DoubleShowType show_type)
{
...
bool GLGizmoColorCut::cut_line_processing() const {
    return m_cut_line_begin != Vec3d::Zero();
}

void GLGizmoColorCut::discard_cut_line_processing()
{
    m_cut_line_begin = m_cut_line_end = Vec3d::Zero();
}

bool GLGizmoColorCut::process_cut_line(SLAGizmoEventType action, const Vec2d &mouse_position)
{
    const Camera &camera = wxGetApp().plater()->get_camera();
...
ColorCutPartSelection::ColorCutPartSelection(
...
// In CutMode::cutTongueAndGroove we use ColorCutPartSelection just for rendering
ColorCutPartSelection::ColorCutPartSelection(const ModelObject *object, int instance_idx_in) : m_instance_idx(instance_idx_in)
{
    add_object(object);
    const ModelVolumePtrs &volumes     = model_object()->volumes;
    const Vec3d            inst_offset = model_object()->instances[m_instance_idx]->get_offset();
...
void ColorCutPartSelection::part_render(const Vec3d *cut_center, const Vec3d *normal)
{
    if (!valid())
        return;
...
void ColorCutPartSelection::add_object(const ModelObject *object)
{
    m_model = Model();
...
bool ColorCutPartSelection::is_one_object() const
{
...
std::vector<Cut::Part> ColorCutPartSelection::get_cut_parts()
{
...
bool ColorCutPartSelection::has_modified_cut_parts()
{
...
void ColorCutPartSelection::toggle_selection(const Vec2d &mouse_pos)
{
    const Camera &camera     = wxGetApp().plater()->get_camera();
...
void ColorCutPartSelection::toggle_selection(int id)
{
    if (id >= 0) {
...
void ColorCutPartSelection::turn_over_selection()
{
