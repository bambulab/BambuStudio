...
const double GLGizmoMove::Offset = 55.0 * 1.15;
#else
const double GLGizmoMove::Offset = 10.0;
#endif
...
Vec3d GLGizmoMove::m_displacement = Vec3d::Zero();


GLGizmoMove::GLGizmoMove(GLCanvas3D& parent, GLGizmoMove::Axis axis)
...
GLGizmoMove::GLGizmoMove(const GLGizmoMove& other)
    : GLGizmoBase(other.m_parent, other.m_icon_filename, other.m_sprite_id)
{
    ;
}

std::string GLGizmoMove::get_tooltip() const
{
    ...
    if (m_axis == X || m_grabbers[0].dragging)
        return "X: " + format(show_position ? position(0) : GLGizmoMove::m_displacement(0), 2);
    else if (m_axis == Y || m_grabbers[0].dragging)
        return "Y: " + format(show_position ? position(1) : GLGizmoMove::m_displacement(1), 2);
    else if (m_axis == Z || m_grabbers[0].dragging)
        return "Z: " + format(show_position ? position(2) : GLGizmoMove::m_displacement(2), 2);
    else
        return "";
}

void GLGizmoMove::render_grabbers_for_picking(const BoundingBoxf3& box) const
{
#if ENABLE_FIXED_GRABBER
    ...
    for (unsigned int i = 0; i < (unsigned int)m_grabbers.size(); ++i) {
        if (m_grabbers[i].enabled) {
            std::array<float, 4> color = picking_color_component(m_gizmo_idx);
...
bool GLGizmoMove::on_init()
{
    m_grabbers.push_back(Grabber());
    return true;
}

void GLGizmoMove::on_start_dragging()
{
    if (m_hover_id != -1) {
        GLGizmoMove::m_displacement = Vec3d::Zero();
        const BoundingBoxf3& box = m_parent.get_selection().get_bounding_box();
        m_starting_drag_position = m_grabbers[0].center;
...
void GLGizmoMove::on_update(const UpdateData& data)
{
    if (m_axis == X)
        GLGizmoMove::m_displacement.x() = calc_projection(data);
    else if (m_axis == Y)
        GLGizmoMove::m_displacement.y() = -1.0f * calc_projection(data);
    else if (m_axis == Z)
        GLGizmoMove::m_displacement.z() = calc_projection(data);
}

void GLGizmoMove::on_render()
{
    ...
    if (m_axis == X)
        ...
        m_grabbers[0].center = { center.x(), center.y() - Offset, center.z() };
    else if (m_axis == Z)
        m_grabbers[0].center = { center.x(), center.y(), center.z() + Offset };
    ...
    if (m_hover_id == -1) {
        ...
        // draw grabbers
        render_grabbers(box);
        render_grabber_extension((Axis)m_axis, box, false);
    }
    else {
        if (m_hover_id == m_gizmo_idx) {
            // draw axis
            glsafe(::glColor4fv(AXES_COLOR[m_axis].data()));
            ::glBegin(GL_LINES);
            ::glVertex3dv(center.data());
            ...
            if (shader != nullptr) {
                ...
                float mean_size = (float)((box.size().x() + box.size().y() + box.size().z()) / 3.0);
#endif
                m_grabbers[0].render(true, mean_size);
                shader->stop_using();
            }
            render_grabber_extension((Axis)m_axis, box, false);
        }
...
void GLGizmoMove::on_render_for_picking()
{
    glsafe(::glEnable(GL_DEPTH_TEST));
    const BoundingBoxf3& box = m_parent.get_selection().get_bounding_box();
    render_grabbers_for_picking(box);
    render_grabber_extension(m_axis, box, true);
}

void GLGizmoMove::render_grabber_extension(Axis axis, const BoundingBoxf3& box, bool picking) const
{
#if ENABLE_FIXED_GRABBER
    float mean_size = (float)(GLGizmoBase::Grabber::FixedGrabberSize);
#else
...
double GLGizmoMove::calc_projection(const UpdateData& data) const
{
