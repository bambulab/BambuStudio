...
bool ImGuiWrapper::horizontal_slider(const char *str_id, int *value, int v_min, int v_max, ImVec2 size, int selection, float scale)
{
    ...
    const ImRect draggable_region(ImVec2(scroll_rect.Min.x, bg_rect.Min.y), ImVec2(scroll_rect.Max.x, bg_rect.Max.y));
    const bool hovered = ImGui::ItemHoverable(draggable_region, id);
    // set active status when mouse hovered.
    if (hovered)
    {
...
bool ImGuiWrapper::vertical_slider(const char* str_id, int* higher_value, int* lower_value, std::string &higher_label, std::string &lower_label,
    int v_min, int v_max, ImVec2 size, int selection, bool one_layer_flag, ImTextureID arrow_id, float scale) 
{
    ImGuiWindow* window = ImGui::GetCurrentWindow();
    ImGui::SetWindowFontScale(1.0f / scale);
    if (window->SkipItems)
    ...
    // set draggable region
    ImRect draggable_region(ImVec2(bg_rect.Min.x, scroll_rect.Min.y), ImVec2(bg_rect.Max.x, scroll_rect.Max.y));;
    ...
    // set active status when mouse hovered in draggble region.
    const bool hovered = ImGui::ItemHoverable(draggable_region, id);
    if (hovered) {
        ImGui::SetActiveID(id, window);
        ImGui::SetFocusID(id, window);
    ...
    float  handle_pos      = lower_draggble_region.Min.y + (lower_draggble_region.Max.y - lower_draggble_region.Min.y) * handle_pos_ratio;
    ...
    handle_pos_ratio       = 1.0f - handle_pos_ratio;
    handle_pos             = higher_draggble_region.Min.y + (higher_draggble_region.Max[1] - higher_draggble_region.Min.y) * handle_pos_ratio;
    ...
    if (!one_layer_flag) {
        ...
        value_changed = slider_behavior(id, selected_h ? higher_draggble_region : lower_draggble_region, v_min, v_max,
        ...
        ImGui::RenderFrame(scroll_line.Min, scroll_line.Max, handle_and_line_col, false, 0.5 * line_offset);
        ...
        *lower_value = *higher_value;
        lower_handle = higher_handle;
        ImVec2 middle_center = lower_handle.GetCenter() * 0.5 + higher_handle.GetCenter() * 0.5;

        // draw grabber
        ImGui::RenderFrame(middle_center - ImVec2(0.5 * fixed_bar_width, 1.0f), middle_center + ImVec2(0.5f * fixed_bar_width + right_blank, 1.0f), handle_and_line_col, false, 1.0f);
        // draw triangle icons
        ...
        ImRect triangle_draggable_region = ImRect(ImVec2(middle_center.x - half_edge * 1.73f, draggable_region.Min.y), ImVec2(middle_center.x, draggable_region.Max.y));
        if (ImGui::ItemHoverable(triangle_handle, id) && context.IO.MouseDown[0]) {
            ImGui::SetActiveID(id, window);
            ImGui::SetFocusID(id, window);
            ImGui::FocusWindow(window);
            slider_behavior(id, triangle_draggable_region, v_min, v_max,
        ...
        //ImGui::ImageButton3(arrow_id, arrow_id, ImVec2(17, 20), ImVec2(0, 0), ImVec2(1, 1), -1, ImVec4(0, 0, 0, 0), ImVec4(1, 1, 1, 1), ImVec2(0, 0), pos_1);
