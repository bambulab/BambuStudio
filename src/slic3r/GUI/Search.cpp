...
void OptionsSearcher::append_options(DynamicPrintConfig *config, Preset::Type type, ConfigOptionMode mode)
{
    ...
    for (std::string opt_key : config->keys()) {
        ...
        if ((type == Preset::TYPE_SLA_MATERIAL || type == Preset::TYPE_PRINTER) && opt_key != "printable_area" && opt_key != "thumbnails")
...
{
    int icon_id = 0;
    for (const std::string &icon : {"cog", "printer", "sla_printer", "spool", "resin"}) m_icon[icon_id++] = ScalableBitmap(parent, icon);
}
