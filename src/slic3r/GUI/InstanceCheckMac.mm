...
-(void)add_observer:(NSString *)version_hash
{
	...
	//NSString *nsver = @"OtherBambuSlicerInstanceMessage" + version_hash;
	NSString *nsver = [NSString stringWithFormat: @"%@%@", @"OtherBambuSlicerInstanceMessage", version_hash];
	...
	NSString *nsver2 = [NSString stringWithFormat: @"%@%@", @"OtherBambuSlicerInstanceClosing", version_hash];
...
void send_message_mac(const std::string &msg, const std::string &version)
{
	...
	//NSString *nsver = @"OtherBambuSlicerInstanceMessage" + [NSString stringWithCString:version.c_str() encoding:[NSString defaultCStringEncoding]];
	NSString *nsver = [NSString stringWithCString:version.c_str() encoding:[NSString defaultCStringEncoding]];
	NSString *notifname = [NSString stringWithFormat: @"%@%@", @"OtherBambuSlicerInstanceMessage", nsver];
...
void send_message_mac_closing(const std::string &msg, const std::string &version)
{
	...
	//NSString *nsver = @"OtherBambuSlicerInstanceMessage" + [NSString stringWithCString:version.c_str() encoding:[NSString defaultCStringEncoding]];
	NSString *nsver = [NSString stringWithCString:version.c_str() encoding:[NSString defaultCStringEncoding]];
	NSString *notifname = [NSString stringWithFormat: @"%@%@", @"OtherBambuSlicerInstanceClosing", nsver];
	[[NSDistributedNotificationCenter defaultCenter] postNotificationName:notifname object:nil userInfo:[NSDictionary dictionaryWithObject:nsmsg forKey:@"data"] deliverImmediately:YES];
}
