                                    ...
                                    const std::vector<std::string>& nonsys_options)
{
    ...
    wxString override_title;
    if (dirty_count > 0)
        override_title = wxString::Format(_L("%d modified setting(s)"), dirty_count);
    else if (nonsys_count > 0)
        override_title = wxString::Format(_L("%d system override(s)"), nonsys_count);
    else
        override_title = _L("No modified settings");
    m_override_title->SetLabel(override_title);
