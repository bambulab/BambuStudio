...
#include "ShortestPathInternal.hpp"
...
std::vector<const PrintInstance*> chain_print_object_instances(const std::vector<const PrintObject*>& print_objects, const Point* start_near)
{
    ...
    object_reference_points.reserve(print_objects.size());
    ...
    std::vector<size_t> ordered = chain_points(object_reference_points, const_cast<Point*>(start_near));
...
Polylines chain_lines(const std::vector<Line>& lines, const double point_distance_epsilon)
{
    ...
    using ClosestPointLookupType = ClosestPointInRadiusLookup<LineEnd, LineEndAccessor>;
    ...
    const double point_distance_epsilon2 = point_distance_epsilon * point_distance_epsilon;
    Polylines out;
