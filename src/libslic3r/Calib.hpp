...
public:
    CalibPressureAdvancePattern(const Calib_Params &params, const DynamicPrintConfig &config, bool is_bbl_machine, const ModelObject &object, const Vec3d &origin);
...
protected:
    ...
    double accel_perimeter() const { return m_config.option<ConfigOptionFloatsNullable>("outer_wall_acceleration")->get_at(m_params.extruder_id); };
    double line_width_first_layer() const { return m_config.get_abs_value("initial_layer_line_width"); };
    double line_width() const {
        double w = m_config.get_abs_value("line_width");
        if (w <= 0)
            w = Flow::auto_extrusion_width(frPerimeter, m_config.option<ConfigOptionFloatsNullable>("nozzle_diameter")->get_at(m_params.extruder_id));
        return w;
    };
