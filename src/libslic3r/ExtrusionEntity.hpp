...
public:
    ...
    // Index of the wall this entity belongs to, counted outwards-in: 0 is the outermost
    // wall, -1 when unknown. Set by the perimeter generators; lets later stages tell
    // outer walls from inner ones without re-deriving the nesting.
    int inset_idx = -1;
...
public:
    ...
    // True if this path was generated by the wave-overhang subsystem; the G-code stage
    // applies the wave-overhang speed and fan overrides when set.
    bool wave_overhang = false;
    // True if this path is solid infill on a floor layer above a wave-overhang region.
    bool wave_overhang_floor = false;
    // True if this path is a wall on a layer that also carries wave-overhang traces.
    // Those walls anchor onto cantilevered traces, so they get their own speed override.
    bool wave_overhang_perimeter = false;
    // True if this path is a wall on a floor layer above a wave-overhang region.
    bool wave_overhang_floor_perimeter = false;
    // 1-based layer distance to the nearest wave layer below, on wave-overhang floor
    // layers; 0 otherwise. The G-code stage ramps floor speeds back to normal over it.
...
public:
    ...
    ExtrusionMultiPath() {}
    // Note: these four do not chain to the ExtrusionEntity copy/move, so everything
    // on the base is dropped here. Only inset_idx is carried across deliberately;
    // propagating m_customize_flag and m_cooling_node as well would be a behavior
    // change, so that pre-existing gap is left alone.
    ExtrusionMultiPath(const ExtrusionMultiPath &rhs) : paths(rhs.paths), m_can_reverse(rhs.m_can_reverse) { inset_idx = rhs.inset_idx; }
    ExtrusionMultiPath(ExtrusionMultiPath &&rhs) : paths(std::move(rhs.paths)), m_can_reverse(rhs.m_can_reverse) { inset_idx = rhs.inset_idx; }
