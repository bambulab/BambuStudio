...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    bool has_delimiters = false;
    {
        ...
        static constexpr const char prusaslicer_gcode_header[] = "; generated by BambuSlicer ";
        //BBS
        ...
        while (std::getline(ifs, header)) {
            ...
            if (strncmp(slic3r_gcode_header, line_c, strlen(slic3r_gcode_header)) == 0) {
                header_found = true;
                break;
            } else if (strncmp(prusaslicer_gcode_header, line_c, strlen(prusaslicer_gcode_header)) == 0 ||
                       strncmp(bambuslicer_gcode_header.c_str(), line_c, strlen(bambuslicer_gcode_header.c_str())) == 0) {
