...
std::vector<std::string> ConfigOptionDef::cli_args(const std::string &key) const
{
	...
	if (this->cli != ConfigOptionDef::nocli) {
        ...
        // Neither '=' nor '!' is used in any of the cli parameters currently defined by BambuSlicer.
...
bool ConfigBase::set_deserialize_raw(const t_config_option_key &opt_key_src, const std::string &value, ConfigSubstitutionContext& substitutions_ctxt, bool append)
{
    ...
    if (optdef->type == coBools && substitutions_ctxt.rule != ForwardCompatibilitySubstitutionRule::Disable) {
    	//FIXME Special handling of vectors of bools, quick and not so dirty solution before BambuSlicer 2.3.2 release.
...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Also the "; generated by BambuSlicer ..." comment line will be removed.
    size_t j = 0;
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Look for Slic3r or BambuSlicer header.
    ...
    bool has_delimiters = false;
    {
        ...
        //BBS
        std::string bambuslicer_gcode_header = "; generated by ";
        ...
        while (std::getline(ifs, header)) {
            ...
            // BBS
            if (strncmp(slic3r_gcode_header, line_c, strlen(slic3r_gcode_header)) == 0) {
                header_found = true;
                break;
            }
        }
        if (!header_found) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << "Not a BambuSlicer / Slic3r PE generated g-code.";
            throw Slic3r::RuntimeError("Not a BambuLab / BambuSlicer / Slic3r PE generated g-code.");
        }
    }
    ...
    if (has_delimiters)
    {
        // BambuSlicer starting with 2.4.0-alpha0 delimits the config section stored into G-code with 
        // ; bambuslicer_config = begin
        // ...
        // ; bambuslicer_config = end
        ...
        while (reader.getline(line))
            if (line == "; bambuslicer_config = end") {
                end_found = true;
                break;
            }
        if (!end_found) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << format("Configuration block closing tag \"; bambuslicer_config = end\" not found when reading %1%", file);
            throw Slic3r::RuntimeError(format("Configuration block closing tag \"; bambuslicer_config = end\" not found when reading %1%", file));
        }
        std::string key, value;
        while (reader.getline(line)) {
            if (line == "; bambuslicer_config = begin") {
                begin_found = true;
                break;
            }
        ...
        if (!begin_found) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << format("Configuration block opening tag \"; bambuslicer_config = begin\" not found when reading %1%", file);
            throw Slic3r::RuntimeError(format("Configuration block opening tag \"; bambuslicer_config = begin\" not found when reading %1%", file));
        }
    }
    else
    {
