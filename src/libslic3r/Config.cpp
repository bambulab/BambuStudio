...
std::vector<std::string> ConfigOptionDef::cli_args(const std::string &key) const
{
	...
	if (this->cli != ConfigOptionDef::nocli) {
        ...
        // Neither '=' nor '!' is used in any of the cli parameters currently defined by BambuStudio.
...
bool ConfigBase::set_deserialize_raw(const t_config_option_key &opt_key_src, const std::string &value, ConfigSubstitutionContext& substitutions_ctxt, bool append)
{
    ...
    if (optdef->type == coBools && substitutions_ctxt.rule != ForwardCompatibilitySubstitutionRule::Disable) {
    	//FIXME Special handling of vectors of bools, quick and not so dirty solution before BambuStudio 2.3.2 release.
...
ConfigSubstitutions ConfigBase::load_from_ini_string_commented(std::string &&data, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    // Also the "; generated by BambuStudio ..." comment line will be removed.
    size_t j = 0;
...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    if (has_delimiters)
    {
        ...
        if (!begin_found) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << format("Configuration block closing tag \"; bambuslicer_config = begin\" not found when reading %1%", file);
            throw Slic3r::RuntimeError(format("Configuration block closing tag \"; bambuslicer_config = begin\" not found when reading %1%", file));
        }
        ...
        if (!end_found) {
            BOOST_LOG_TRIVIAL(error) << __FUNCTION__ << format("Configuration block opening tag \"; bambuslicer_config = end\" not found when reading %1%", file);
            throw Slic3r::RuntimeError(format("Configuration block opening tag \"; bambuslicer_config = end\" not found when reading %1%", file));
        }
    }
    else
    {
