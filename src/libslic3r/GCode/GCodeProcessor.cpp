...
{
    auto format_line_M104 = [&physical_extruder_map = this->physical_extruder_map, &extruder_nozzle_count = this->extruder_max_nozzle_count](int target_temp, int target_extruder, bool skippable, int next_filament_idx, const std::string& comment = std::string()) {
        std::string buffer;
        skippable &= (extruder_nozzle_count[target_extruder] > 1);
        if (skippable) {
            buffer += "M632 S " + std::to_string(next_filament_idx) + " N R\n";
        }
        buffer += "M104";
        if (target_extruder != -1)
    ...
    if (apply_cooling_when_partial_free) {
        ...
        inserted_operation_lines[block.partial_free_lower_id].emplace_back(format_line_M104(curr_temp, block.extruder_id, false, block.next_filament_id, "Multi extruder pre cooling in post extrusion"), TimeProcessor::InsertLineType::PreCooling);
    }
    ...
    if (pre_cooling && !pre_heating) {
        ...
        if (target_temp >= curr_temp)
            return;
        inserted_operation_lines[block.free_lower_gcode_id].emplace_back(format_line_M104(target_temp, block.extruder_id, false, block.next_filament_id, "Multi extruder pre cooling"), TimeProcessor::InsertLineType::PreCooling);
        return;
    }
    if (!pre_cooling && pre_heating) {
        ...
        if (heating_move_iter == move_iter_lower) {
            inserted_operation_lines[block.free_lower_gcode_id].emplace_back(format_line_M104(target_temp, block.extruder_id, true, block.next_filament_id, "Multi extruder pre heating"), TimeProcessor::InsertLineType::PreHeating);
        }
        else {
            --heating_move_iter;
            heating_move_iter = adjust_iter(heating_move_iter, move_iter_lower, move_iter_upper, false);
            inserted_operation_lines[heating_move_iter->gcode_id].emplace_back(format_line_M104(target_temp, block.extruder_id, true, block.next_filament_id, "Multi extruder pre heating"), TimeProcessor::InsertLineType::PreHeating);
        }
        return;
    }
    ...
    if (real_delta_temp == 0)
        return;
    inserted_operation_lines[block.free_lower_gcode_id].emplace_back(format_line_M104(curr_temp - real_delta_temp, block.extruder_id, false, block.next_filament_id, "Multi extruder pre cooling"), TimeProcessor::InsertLineType::PreCooling);
    inserted_operation_lines[heating_move_iter->gcode_id].emplace_back(format_line_M104(target_temp, block.extruder_id, true, block.next_filament_id, "Multi extruder pre heating"), TimeProcessor::InsertLineType::PreHeating);
}
