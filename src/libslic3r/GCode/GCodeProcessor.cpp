...
const std::vector<std::pair<GCodeProcessor::EProducer, std::string>> GCodeProcessor::Producers = {
    //BBS: BambuSlicer is also "prusa". Otherwise the time estimation didn't work.
    //FIXME: Workaround and should be handled when do removing-prusa
    { EProducer::BambuSlicer, SLIC3R_APP_NAME },
    { EProducer::BambuSlicer, "generated by BambuSlicer" },
...
void GCodeProcessor::process_file(const std::string& filename, std::function<void()> cancel_callback)
{
    ...
    // parse the gcode file to detect its producer
    {
        ...
        // if the gcode was produced by BambuSlicer,
        // extract the config from it
        if (m_producer == EProducer::BambuSlicer || m_producer == EProducer::Slic3rPE || m_producer == EProducer::Slic3r) {
            DynamicPrintConfig config;
            config.apply(FullPrintConfig::defaults());
            // Silently substitute unknown values by new ones for loading configurations from BambuSlicer's own G-code.
...
void GCodeProcessor::process_tags(const std::string_view comment, bool producers_enabled)
{
    ...
    if (!producers_enabled || m_producer == EProducer::BambuSlicer) {
        // height tag
...
bool GCodeProcessor::process_producers_tags(const std::string_view comment)
{
    ...
    case EProducer::BambuSlicer: { return process_prusaslicer_tags(comment); }
