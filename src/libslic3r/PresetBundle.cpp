    ...
    const std::string &path, const std::string &vendor_name, LoadConfigBundleAttributes flags, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    for (auto& subfile : process_subfiles)
    {
        ...
        if (!reason.empty()) {
            ...
            throw ConfigurationError((boost::format("Failed loading configuration file %1%\nSuggest deleting the contents of %2% and restarting the app.") % subfile_path % path).str());
        }
    }
...
VendorProfile::PrinterModel PresetBundle::load_vendor_configs_from_json(const std::string &path)
{
    ...
    try {
        ...
        throw ConfigurationError((boost::format("Failed loading configuration file %1%\nSuggest deleting the contents of %2% and restarting the app.") % err.what() % path).str());
    }
    return model;
}
