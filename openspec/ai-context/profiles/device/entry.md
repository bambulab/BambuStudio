...
3. 用户在当前会话里明确要求进入设备域（例如「按设备域处理」「使用设备域 XX 技能」）。

任意一路打开即启用，不要求三路同时满足。

## 启用后的工作方式

在本仓库中处理设备相关开发、分析、排查或文档整理任务时，应将 `/openspec/` 下的设备知识视为优先上下文来源。

执行原则：

1. 不直接全量扫描 `/openspec/` 下的设备目录
2. 先按入口和索引建立上下文
3. 遇到设备域 `OpenSpec` 任务时，必须先进入 `/openspec/rules/device/03-openspec规则.md`
4. 再根据任务类型进入规则、技能或专题文档
5. 回答、实现和分析优先与 `/openspec/` 中已有设备约束保持一致
