...
| Filament mapping config synchronization | partial | `libslic3r_config_tests`, `print_filament_mapping_smoke_tests` | `FilamentMappingConfigSync::sync_maps_to_config()` now owns the config synchronization rules in config-core, including explicit volume/nozzle maps and default volume-map derivation. `Print::update_filament_maps_to_config()` remains covered by the heavier mapping smoke as the integration API for placeholder parser updates, cache refresh, and auto-map state. The heavy mapping path filter is scoped to the `Print` API integration surface; `PrintFilamentCompatibility.cpp` changes are covered by apply/validate smoke instead. |
