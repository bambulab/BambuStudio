...
| `test_skirt_brim.cpp` | brim tool-selection through exported G-code | skip/manual | future G-code export smoke only if behavior is revalidated | Legacy cases are inside `#if 0` and note a real historical mismatch; do not revive without product confirmation and a stable export harness. The parser-only helper intent for associating the first real extrusion move with the active `T` command is covered in `gcode_reader_smoke_tests`; actual brim tool ownership remains unclaimed by PR smoke. |
...
| `test_printgcode.cpp` | G-code preamble unit/positioning setup | done | `gcodewriter_smoke_tests` | Migrated through the `GCodeWriter::preamble()` responsibility boundary by asserting the stable absolute-positioning and millimeter-unit commands directly. Full export comment formatting and surrounding generated output remain in the manual/nightly export bucket. |
...
| Filament mapping validate boundary | done | `print_filament_mapping_smoke_tests`, `print_apply_validate_smoke_tests` | `libslic3r_print_filament_mapping_core` no longer compiles real `PrintValidate.cpp`, `GCodeLayerOrdering.cpp`, or `PrintFilamentCompatibility.cpp`; the mapping smoke only needs `Print::apply()` plus mapping APIs, and `PrintFilamentMappingCompat.cpp` guards accidental `Print::validate()` / `Print::process()` calls as unreachable. Real validate behavior remains covered by `print_apply_validate_smoke_tests`. |
| Filament mapping model path-filter boundary | done | `model_basic_smoke_tests`, `print_apply_validate_smoke_tests`, `print_filament_mapping_smoke_tests` | The mapping smoke path filter no longer treats `Model.cpp`, `Model.hpp`, or `TriangleMesh.hpp` as filament-mapping triggers. Model assembly and single-mesh `Print::apply()` initialization remain covered by `model_basic_smoke_tests` / `print_apply_validate_smoke_tests`; `print_filament_mapping_smoke_tests` stays scoped to mapping API/config/state changes. GitHub Actions run `27116683501` validated commit `df47aaa1f`: only the filament mapping smoke scope was enabled for the path-filter change, `print_filament_mapping_smoke_tests` built in about 13m08s, ran successfully, and all other smoke targets were skipped. |
...
| G-code writer CI scope | done | `gcodewriter_smoke_tests` | GitHub Actions run `27115993164` validated commit `c2a8bbb1d`: only the GCodeWriter smoke scope was enabled for `tests/fff_print/test_gcodewriter_smoke.cpp`, `gcodewriter_smoke_tests` built in about 1m42s, ran successfully, and all heavy print/export smoke targets were skipped by path filters. |
| G-code reader parser state | done | `gcode_reader_smoke_tests` | `libslic3r_gcode_reader_core` owns parser-only coverage extracted from legacy export tests: final Z tracking, complete-object Z reset detection, extrusion/travel classification, comments, tool selection, and first-extrusion active-tool tracking. This keeps high-value parser regressions in PR smoke without linking `Print::export_gcode()` or changing export behavior. |
| G-code reader CI scope | done | `gcode_reader_smoke_tests` | GitHub Actions run `27114641161` validated commit `03460669d`: only the GCodeReader smoke scope was enabled for `tests/fff_print/test_gcode_reader_smoke.cpp`, `gcode_reader_smoke_tests` built in about 1m38s, ran successfully, and all other smoke targets were skipped by path filters. GitHub Actions run `27116188585` validated commit `bdc153078`: only the GCodeReader smoke scope was enabled, `gcode_reader_smoke_tests` built in about 1m37s, ran successfully, and heavy print/export smoke targets were skipped. |
