...
SCENARIO("Polygon convex/concave detection", "[Geometry]"){
    GIVEN(("A Square with dimension 100")){
		...
		// OrcaSlicer also checks a threshold of exactly a right angle. It changed the library to make that
		// case exclusive (cos(nextafter(threshold)) in filter_convex_concave_points_by_angle_threshold);
		// BambuStudio still counts a right angle at a PI/2 threshold, so that case is left out here.
    }
