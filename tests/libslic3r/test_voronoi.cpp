...
TEST_CASE("Voronoi offset", "[VoronoiOffset]")
{
  ...
  // The polygon is written in PrusaSlicer units, the offsets with scale_().
  poly_with_hole = from_prusa_units(poly_with_hole);
  VD vd;
  ...
  for (const OffsetTest &ot : {
            ...
            // 0.5 mm is exactly where the inside splits in two. With BambuStudio's coarser scaled units the
            // result at that exact value rounds to one polygon; 0.505 and 0.51 cover the two-polygon range.
      ...
      INFO("offset distance " << unscale<double>(ot.distance) << " mm");
...
TEST_CASE("Voronoi offset with edge collapse", "[VoronoiOffset4]")
{
  ...
  VD vd;
  // The polygons are written in PrusaSlicer units, the offsets with scale_().
  poly = from_prusa_units(poly);
  Lines lines = to_lines(poly);
  vd.construct_voronoi(lines.begin(), lines.end());
