...
#
# macOS variant: builds the osx_cocoa port (no GTK), and wxWebView is
# wired against the system WebKit framework (no webkit2gtk dep needed).
#
...
# paths to point inside the pixi env so the copy actually works. On macOS
# the port is osx_cocoa instead of gtk3.
wx_variant="osx_cocoa-unicode-static-3.1"
wx_config_real="$CONDA_PREFIX/lib/wx/config/$wx_variant"
ln -sfn "../lib/wx/config/$wx_variant" "$CONDA_PREFIX/bin/wx-config"
# BSD sed needs an explicit (empty) backup suffix after -i.
sed -i '' "s|$src|$CONDA_PREFIX|g" \
    "$wx_config_real" \
    "$CONDA_PREFIX/lib/wx/include/$wx_variant/wx/setup.h"

echo "wxWidgets installed to $CONDA_PREFIX"
echo "  wx-config -> $(readlink "$CONDA_PREFIX/bin/wx-config")"
